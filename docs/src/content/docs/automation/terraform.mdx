...
- terraform/ network/hardware
  ...
  - bunny.tf configures the CDN for media files
...
import terraformBunny from '/../terraform/bunny.tf?raw';

<Code code={terraformBunny} lang="tf" title="terraform/bunny.tf" />

Terraform uses this file to configure the "Storage Zone" (bucket) to store media files in and the "Pull Zone" (CDN) to serve them.
## State file
...
Media files are served through the Bunny.net CDN, and that needs an API token to configure.
